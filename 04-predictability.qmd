...
p_2023 & labs(caption = NULL)
```

```{r}
#| label: fig-punc-test
#| fig-cap: just plot them
...
punc_bra |> 
  ...
  ggplot() + 
  geom_line(aes(x = YEAR, y = WITHIN_M15P15, group = APT, color = APT)) +
  geom_point(aes(x = YEAR, y = WITHIN_M15P15, group = APT, color = APT)) +
  labs(caption = "Graph developed by Rafael")
```
...
```{r}
#| label: fig-early-vs-late-arrivals2
#| fig-cap: Change of share of early and late arrivals (2019 vs 2022)
#| fig-height: 2.5
#| layout-nrow: 3

p2019 + plot_layout(guides = "collect") & 
  theme(legend.position = 'top')
p2022
p2023 
```

::: {#fig-myplot layout-ncol=1}
